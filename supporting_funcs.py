...
def plookup(ccode, pcode, rempart):
    ...
    # checking all elements not to be letters
    for ch in ccode:
        checkthiskidisstupidornot1 = ch.isalpha()
        if checkthiskidisstupidornot1:
            # This is ELON MUSK
            print("\033[36m" + "Wrong...Not Letters, only digits :)")
            exit()
    checkthiskidisstupidornot2 = '+' in ccode
    if checkthiskidisstupidornot2:
        # This is ElOn MuSk
        print("\033[36m" + "Wrong...Not Letters, only digits :)")
        exit()
    country = countries[str(ccode)]
    checkthiskidisstupidornot31 = ' ' in rempart
    if checkthiskidisstupidornot31:
        # This is ElOn MuSk
        print("\033[36m" + "Wrong...No Space, only digits :)")
        exit()
    checkthiskidisstupidornot3 = ' ' in pcode
    if checkthiskidisstupidornot3:
        # This is ElOn MuSk
        print("\033[36m" + "Wrong...No Space, only digits :)")
        exit()
    for ch in pcode:
        checkthiskidisstupidornot4 = ch.isalpha()
        if checkthiskidisstupidornot4:
            # This is ELON MUSK
            print("\033[36m" + "Wrong...Not Letters, only digits :)")
            exit()
    provider = "None"
    ...
    cforp = "Bruh"
    if not ruseriousrnnow:
        cforp = providers[country]
    wtfisthis = isinstance(cforp, str)
    if wtfisthis:
        provider = cforp
            ...
            if isx:
                if len(k) != len(pcode):
                    ...
                    if cnt >= 1:
                        provider = v
                        break
    print("For results : +" + "\033[31m" + str(ccode) + pcode + rempart)
    print("\033[36m" + "Country : " + "\033[32m" + country)
    print("\033[36m" + "Provider : " + "\033[32m" + provider)
